...
public:
  ...
  void insert(Seqr_idx, Step_idx, Sequencable auto&&);
  void update(Seqr_idx, Step_idx, Sequencable auto&&);
  template <Sequencable Event_t> void erase(Seqr_idx, Step_idx);

  void start_all(Sequencer_time_point = Sequencer_clock::now());
  void stop_all();

  void start(Seqr_idx, Sequencer_time_point = Sequencer_clock::now());
  void stop(Seqr_idx);
