...
User_interface<T_event_params>::User_interface(
    ...
    : controller_(controller), gui_(std::make_unique<gui::Gui>()) {}

template <typename T_event_params>
void User_interface<T_event_params>::init() {
  // Initialize GUI (GTK will be initialized in gui::Gui)
  // For now, just mark as ready to run
  running_ = false;
}
