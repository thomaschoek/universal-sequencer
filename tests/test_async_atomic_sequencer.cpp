...
public:
    ...
    std::vector<std::pair<int, std::chrono::steady_clock::time_point>> get_handled_events() const {
        std::lock_guard<std::mutex> lock(mutex_);
...
private:
    ...
    std::vector<std::pair<int, std::chrono::steady_clock::time_point>> handled_events_;
...
TEST_CASE("Async AtomicSequencer timing precision", "[async_sequencer]") {
    ...
    auto handler_state = std::make_shared<std::vector<std::pair<int, std::chrono::steady_clock::time_point>>>();
    ...
    std::vector<std::pair<int, std::chrono::steady_clock::time_point>> handled_events;
    {
        std::lock_guard<std::mutex> lock(*handler_mutex);
...
TEST_CASE("Async processing doesn't block sequencer timing", "[async_sequencer]") {
    atomic_deque::AtomicDeque<TestEvent> sequence;
    ...
    auto handler = [handler_state, handler_mutex](TestEvent&& event) {
        std::this_thread::sleep_for(std::chrono::milliseconds(50)); // Slow handler to test non-blocking
