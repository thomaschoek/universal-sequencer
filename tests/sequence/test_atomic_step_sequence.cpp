...
Step createStep(double offset_seconds, double length_seconds, std::vector<double> params = {}) {
    Step step;
...
TEST_CASE("AtomicStepSequence inheritance from AtomicDeque", "[atomic_step_sequence]") {
    AtomicStepSequence sequence(0);
...
TEST_CASE("AtomicStepSequence complex step patterns", "[atomic_step_sequence]") {
    AtomicStepSequence sequence(0);
...
TEST_CASE("AtomicStepSequence thread safety", "[atomic_step_sequence]") {
    ...
    SECTION("Concurrent step() calls") {
        ...
        for (int t = 0; t < num_threads; ++t) {
            futures.push_back(std::async(std::launch::async, [&sequence, steps_per_thread]() {
    ...
    SECTION("Concurrent modification and step() calls") {
        ...
        futures.push_back(std::async(std::launch::async, [&sequence, &stop_flag]() {
            ...
            while (!stop_flag.load()) {
                sequence.push_back(createStep(0.1, 0.1, {static_cast<double>(added + 100)}));
                added++;
