...
int main() {
  ...
  Synthesizer synth_1{synth_out_1}, synth_2{synth_out_2};
  {
    ...
    std::function<void(OscillationEvent&&)> handler_1 =
        [&synth_1](const OscillationEvent&& event) {
          std::ignore = std::async(
              std::launch::async, [&synth_1, event]() { synth_1.play(event); });
        };
    ...
    seqr.add_seq<OscillationEvent>(handler_1, std::move(steps));
    seqr.add_seq<OscillationEvent>(handler_2, std::move(reverse_steps));
  }
